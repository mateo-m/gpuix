...
- `selectable_text(..)` for content. It registers into the per-frame selection
  registry and installs the window mouse and key listeners
- `chrome_text(..)` for line numbers, language tags and file headers. It is painted
...
- `packages/native/index.d.ts`, the TypeScript type declarations
- `packages/native/index.js`, the Node.js loader/binding glue
- `packages/native/*.node`, the compiled native binary

To update the exposed TypeScript calls, edit the Rust source files in `packages/native/src/` (add/modify `#[napi]` structs, methods, functions), then run `bun run build` in `packages/native` to regenerate.
...
- [opentui](https://github.com/anomalyco/opentui), a terminal UI with React (reconciler reference)
- [create-gpui-app](https://github.com/zed-industries/create-gpui-app), the official GPUI starter template
