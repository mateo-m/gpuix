...
`translateY` and `opacity` ranges for the old side and the new side. Percent
lengths resolve against the size of the named element, so
...
Limits in this version: a name that only leaves paints nothing, and the
frozen copy takes no input.
