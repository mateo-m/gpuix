...
pub(super) fn build_element(
    ...
    let built = match element.element_type.as_str() {
        "div" => {
            ...
            build_div(element, style, resolved.clone(), motion.as_ref(), position, ctx, window, cx)
        }
        "text" => {
            ctx.custom_registry.destroy(id);
            build_text(element, style, resolved.clone(), motion.as_ref(), position, ctx, window, cx)
        }
        "virtual-list" => {
            ctx.custom_registry.destroy(id);
